...
import com.britt.listi.usecase.AddItem
import com.britt.listi.usecase.GetAndJoinConversation
import com.britt.listi.usecase.SendText
...
fun main() {
    ...
    embeddedServer(Netty, port = BASE_BAKEND_PORT, host = BASE_BACKEND_HOST) {
        routing {
            ...
            get("/sample/item/{item_name}") {
                val paramItem = call.parameters["item_name"]
                addItem(paramItem)
            }

            get("/sample/message/{item}/{text}") {
                val paramText = call.parameters["text"]
                val paramItem = call.parameters["item"]
                sendItemMessage(paramItem, paramText)
            }
        }
    }.start(wait = true)
}

suspend fun PipelineContext<Unit, ApplicationCall>.sendItemMessage(
    paramItem: String?,
    paramText: String?
) {

    if (paramText != null && paramItem != null) {
        SendText().execute(null, itemName = paramItem, message = paramText)
        call.respondText("Sending message: $paramText , to: $paramItem...", ContentType.Text.Plain)
    }
}

private suspend fun PipelineContext<Unit, ApplicationCall>.addItem(paramItem: String?) {
    paramItem?.let {
        AddItem().execute(it)
        call.respondText("Adding item: $paramItem...", ContentType.Text.Plain)
    }
}
